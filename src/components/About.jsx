...
import { FiBookOpen, FiActivity, FiLayers, FiCheck } from 'react-icons/fi';
...
export default function About() {
  ...
  const stats = [
    { number: '10+', label: 'Projects Completed', icon: <FiLayers className="text-primary w-6 h-6" /> },
    ...
    { number: '5+', label: 'Technologies Studied', icon: <FiActivity className="text-primary w-6 h-6" /> },
  ];
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 items-center mb-16">
          ...
          >
            ...
            <p className="text-text-gray leading-relaxed text-base md:text-lg">
              I am a BSc Computer Science graduate with a deep passion for designing and building highly interactive, scalable web and mobile application experiences. I love bridging frontend aesthetics with highly optimized backend APIs.
            </p>

            {/* Showcase Statistics Highlights */}
            <div className="flex flex-col space-y-4 pt-2">
              <h4 className="text-sm font-bold tracking-wider text-white/95 uppercase font-space">
                Technical Highlights & Showcase:
              </h4>
              <ul className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-3.5">
                {[
                  "2 Real-world Projects Showcase",
                  "15+ Flutter Mobile Applications",
                  "2 Full-Stack Web Applications",
                  "PostgreSQL & Supabase Database Designs",
                  "Maps & Real-time Location Services",
                ].map((statItem, idx) => (
                  <li key={idx} className="flex items-start space-x-3 text-text-gray">
                    <span className="flex-shrink-0 mt-0.5 w-5 h-5 rounded-full bg-primary/10 border border-primary/20 flex items-center justify-center text-primary">
                      <FiCheck className="w-3 h-3" />
                    </span>
                    <span className="text-sm font-medium leading-relaxed">{statItem}</span>
                  </li>
                ))}
              </ul>
            </div>
